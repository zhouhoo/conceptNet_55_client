...
class Path:
    ...
    def __init__(self, concepts=[], relations=[]):
        ...
        self.assertions = self._create_assertions() if len(concepts) > 0 and len(relations) == len(
            concepts) - 1 else None
    ...
    def get_all_tuples_of_concepts(self):
        ...
        for relation in self.relations:
            ...
            for e in edges:
                ...
                graph.add_node(e.end['label'])
                graph.add_edge(e.start['label'], e.end['label'])
                graph[e.start['label']][e.end['label']]['relation'] = relation
